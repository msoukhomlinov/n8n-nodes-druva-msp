...
function getRuntimeRequire(): { runtimeReq: any; diagnostic: string | null } {
  ...
  const diagnostic =
    `[DruvaMspAiTools] No runtime anchor found. Tried: ${ANCHOR_CANDIDATES.join(", ")}. ` +
    `Errors: ${errors.join(" | ")}`;
...
if (runtimeReq) {
  try {
    const coreTools = runtimeReq("@langchain/core/tools") as Record<
      string,
      unknown
    >;
    ...
    console.warn(
      "[DruvaMspAiTools] Failed to resolve @langchain/core/tools from runtime require:",
      e,
    );
  }
  ...
  try {
    ...
    console.warn(
      "[DruvaMspAiTools] Failed to resolve zod from runtime require:",
      e,
    );
  }
}
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const RuntimeDynamicStructuredTool: DynamicStructuredToolCtor =
  new Proxy(function () {} as unknown as DynamicStructuredToolCtor, {
