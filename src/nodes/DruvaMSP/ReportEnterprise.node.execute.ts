...
function addDateFilters(
  ...
  // Period-based and snapshot reports do not support lastUpdatedTime date filters
  else if (
    operation === "getM365StorageConsumptionReport" ||
    operation === "getM365UserCountAndStatus" ||
    operation === "getM365LicenseUsage" ||
    operation === "getGoogleUserCountAndStatus" ||
    operation === "getGoogleLicenseUsage" ||
    operation === "getGoogleStorageConsumption"
  ) {
    return;
  }
...
/**
 * Execute the Enterprise Workloads Report operations
 */
export async function executeReportEnterpriseOperation(
  this: IExecuteFunctions,
  ...
  const limit = returnAll
    ? 0
    : (this.getNodeParameter("limit", 0, 100) as number);
  ...
  for (let i = 0; i < items.length; i++) {
    try {
      ...
      const pageSize = limit || API_MAX_PAGE_SIZE;
      ...
      const filterBy =
        ((requestBody.filters as IDataObject).filterBy as IDataObject[]) || [];
      ...
      if (operation === "getAlertHistoryReport") {
        ...
        if (filterByAlertSeverity) {
          const alertSeverity = this.getNodeParameter(
            "alertSeverity",
            0,
            [],
          ) as string[];
        ...
        if (filterByBackupStatus) {
          const backupStatus = this.getNodeParameter(
            "backupStatus",
            0,
            [],
          ) as string[];
        ...
        if (filterByWorkloadTypes) {
          ...
          if (workloadTypes.length > 0) {
            filterBy.push(
              createFilter("backupContent", "CONTAINS", workloadTypes),
            );
          }
        }
        ...
        // Add additional filters specific to this operation using the structured format
        const filterByDRPlanIds = this.getNodeParameter(
          "filterByDRPlanIds",
          0,
          false,
        ) as boolean;
        if (filterByDRPlanIds) {
          const drPlanIds = this.getNodeParameter(
            "drPlanIds",
            0,
            [],
          ) as string[];
        ...
        if (filterByReplicationStatus) {
          const replicationStatus = this.getNodeParameter(
            "replicationStatus",
            0,
            [],
          ) as string[];
          if (replicationStatus.length > 0) {
            filterBy.push(
              createFilter("status", "CONTAINS", replicationStatus),
            );
          }
        }
        ...
        if (filterByResourceStatus) {
          const resourceStatus = this.getNodeParameter(
            "resourceStatus",
            0,
            [],
          ) as string[];
          if (resourceStatus.length > 0) {
            ...
            filterBy.push(
              createFilter("backupEnabled", "EQUAL", resourceStatus[0]),
            );
          }
        }
        ...
        if (filterByResourceTypes) {
          ...
          if (resourceType.length > 0) {
            filterBy.push(
              createFilter("backupsetType", "CONTAINS", resourceType),
            );
          }
        }
        ...
        if (filterByWorkloadName) {
          const workloadNames = this.getNodeParameter(
            "workloadName",
            0,
            [],
          ) as string[];
          if (workloadNames.length > 0) {
            filterBy.push(
              createFilter("workloadName", "CONTAINS", workloadNames),
            );
          }
        }
        ...
        const endpointMap: Record<string, string> = {
          ...
          getGoogleUserRestoreActivity:
            "/msp/reporting/v1/reports/mspGoogleUserRestoreActivity",
          getGoogleUserWorkload:
            "/msp/reporting/v1/reports/mspGoogleUserWorkload",
        };
        endpoint = endpointMap[operation] || "";
      }

      // All operations use token-based pagination with 'data' as the response key
      responseData = await druvaMspApiRequestAllReportItems.call(
        this,
        endpoint,
        requestBody,
        "data",
